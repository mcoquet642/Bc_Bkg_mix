...
//_________
   ...
   Double_t mum_jpsi_pt, mum_jpsi_eta, mum_jpsi_sig, mum_jpsi_ip, mum_jpsi_vtxZ;
   Double_t mup_jpsi_pt, mup_jpsi_eta, mup_jpsi_sig, mup_jpsi_ip, mup_jpsi_vtxZ;
   Double_t dep1, dep2;
   gROOT->cd();
   ...
   for (Int_t i=0; i<nb_muon_Pip; i++){
	...
	theta1=2*atan(exp(-mum_jpsi_eta));	
	vtxr1=sqrt(origX_m*origX_m+origY_m*origY_m);
	l1=vtxr1/tan(theta1);
	dist1=sgn(mum_jpsi_vtxZ-l1)*mum_jpsi_ip*l1/vtxr1;
	...
	p1[3]=e_m;
   	for (Int_t j=0; j<blist_hadrons[3*index_ev]; j++) {
		...
		pz=ptot[2];
		e=ptot[3];
		pt=sqrt(ptot[0]*ptot[0]+ptot[1]*ptot[1]);
		y=log((e+ptot[2])/(e-ptot[2]))*0.5;
		ev=index_ev;
		...
		theta2=2*atan(exp(-mup_jpsi_eta));	
		vtxr2=sqrt(origX_p*origX_p+origY_p*origY_p);
		l2=vtxr2/tan(theta2);
		dist2=sgn(mup_jpsi_vtxZ-l2)*mup_jpsi_ip*l2/vtxr2;
		...
		dep_av=(dist1+dist2)/2;
		dep_av=dep_min+(dep_max-dep_min)*tan(alpha2)/(tan(alpha1)+tan(alpha2));
		double phi = r4.Uniform(2*3.1415926);
		origx = cos(phi)*vtxr1*dep_av/l1; 
		origy = sin(phi)*vtxr1*dep_av/l1; 
	...
	for (Int_t j=0; j<blist_hadrons[3*index_ev+1]; j++) {
		...
		theta2=2*atan(exp(-mup_jpsi_eta));	
		vtxr2=sqrt(origX_p_2*origX_p_2+origY_p_2*origY_p_2);
		l2=vtxr2/tan(theta2);
		dist2=sgn(mup_jpsi_vtxZ-l2)*mup_jpsi_ip*l2/vtxr2;
	...
	for (Int_t j=0; j<blist_hadrons[3*index_ev+2]; j++) {
		...
		theta2=2*atan(exp(-mup_jpsi_eta));	
		vtxr2=sqrt(origX_p_3*origX_p_3+origY_p_3*origY_p_3);
		l2=vtxr2/tan(theta2);
		dist2=sgn(mup_jpsi_vtxZ-l2)*mup_jpsi_ip*l2/vtxr2;
   ...
   for (Int_t i=0; i<nb_muon_Dp; i++){
	...
	theta1=2*atan(exp(-mum_jpsi_eta));	
	vtxr1=sqrt(origX_m_2*origX_m_2+origY_m_2*origY_m_2);
	l1=vtxr1/tan(theta1);
	dist1=sgn(mum_jpsi_vtxZ-l1)*mum_jpsi_ip*l1/vtxr1;
   ...
   for (Int_t i=0; i<nb_muon_K; i++){
	...
	theta1=2*atan(exp(-mum_jpsi_eta));	
	vtxr1=sqrt(origX_m_3*origX_m_3+origY_m_3*origY_m_3);
	l1=vtxr1/tan(theta1);
	dist1=sgn(mum_jpsi_vtxZ-l1)*mum_jpsi_ip*l1/vtxr1;
